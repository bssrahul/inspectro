...
@endif
	...
	<div id='TextBoxesGroup'>
			...
			<div id="TextBoxDiv1" class="form-group address hclass sh answerOptionBox">
				...
				<div class="form-group">
					{!! Form::label('short_name[0]', 'Answer short Name:') !!}
					<input type="text" name="short_name[0]" class="form-control required" value="<?php if(!empty($answer['short_name'])){ echo 	$answer['short_name']; }?>">
					{!! $errors->first('short_name[0]', '<div class="text-danger">:message</div>') !!}
				</div>
				...
				<div class="form-group_part">
							...
							<select name="next_question_id[0]" class = 'form-control required' >
				...
				<div class="form-group_part">
							...
							&nbsp;&nbsp;&nbsp;
							<input type="number" name="sort[0]" class = 'form-control'  value = "<?php if(@$sortIndex){ echo @$sortIndex;}else{ echo @$answer['sort'];} ?>">
...
<script src="{!! admin_asset('components/jquery/jquery.validate.js') !!}" type="text/javascript"></script>
	...
	counter++;	
	<?php if(@$sortIndex){ ?>
	var newCounter= counter + parseInt("<?php echo $sortIndex ;?>");
	<?php } ?>
	//alert(newCounter);
	...
	newTextBoxDiv.after().html('<div class="form-group answerInput"><label>Answer : </label>' +
		  ...
		  '<div class="form-group"><label for="short_name['+counter+']"> Answer Sort Name:</label><input type="text" '+
		 ' name="short_name['+counter+']" class="form-control required" value="<?php if(!empty($answer['short_name'])){ echo 	$answer['short_name']; }?>">	</div>'+
		...
		'<br><input type="radio" name="custom_answer['+counter+']" value="date" <?php if((!empty($answer['custom_answer'])) && ($answer['custom_answer'] == 'date')){ echo "checked=checked";} ?> ><span><?php echo" Date ";?> </span>'+'<br><span> <br> <?php echo"( If you want to use Custom Answer, Please check above box )";?> </span></div>'+				
							
		  '<div class="form-group_part"><label for="next_question_id['+counter+']"> Next Qusetion  :</label><select name="next_question_id['+counter+']" class = "form-control required"><?php foreach($nextQuestionArr as $k =>$questions){ ?>'+
		  ...
		  '&nbsp;&nbsp;&nbsp;<input type="number" name="sort['+counter+']" class = "form-control" value="'+newCounter+'"><div class="text-danger"></div></div> <div class="form-group_part2"><label for="option_description['+counter+']"> Option Description :</label>'+
